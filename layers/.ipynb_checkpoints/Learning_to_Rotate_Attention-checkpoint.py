...
class ComplexAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, query_omegas, query_thetas, key_omegas, key_thetas,attn_mask):
        ...
        queries_angles = query_omegas * self.query_pos + query_thetas # (B, L, H, M)
        keys_angles = key_omegas * self.key_pos + key_thetas # (B, S, H, M)

        queries_sinusoidal = [torch.stack([queries_angles.cos(), -queries_angles.sin()], dim=4), torch.stack([queries_angles.sin(), queries_angles.cos()], dim=4)] # (B, L, H, M, 2) of 2
        keys_sinusoidal = [torch.stack([keys_angles.cos(), -keys_angles.sin()], dim=4), torch.stack([keys_angles.sin(), keys_angles.cos()], dim=4)] # (B, S, H, M, 2) of 2

        complex_queries = torch.stack(torch.chunk(queries, 2, dim=-1), dim=4) # (B, L, H, E//2, 2)
        complex_keys = torch.stack(torch.chunk(keys, 2, dim=-1), dim=4) # (B, S, H, E//2, 2)

        queries_real = torch.einsum('blhmc,blhec->blhmec', queries_sinusoidal[0], complex_queries).sum(dim=-1) # (B, L, H, M, E//2)
        keys_real = torch.einsum('bshmc,bshec->bshmec', keys_sinusoidal[0], complex_keys).contiguous().sum(dim=-1) # (B, S, H, M, E//2)

        queries_imaginary = torch.einsum('blhmc,blhec->blhmec', queries_sinusoidal[1], complex_queries).sum(dim=-1) # (B, L, H, M, E//2)
        keys_imaginary = torch.einsum('bshmc,bshec->bshmec', keys_sinusoidal[1], complex_keys).contiguous().sum(dim=-1) # (B, S, H, M, E//2)

        scale = self.scale or 1. / sqrt(E)

        scores = (torch.einsum("blhme,bshme->bhls", queries_real, keys_real) + torch.einsum("blhme,bshme->bhls", queries_imaginary, keys_imaginary)) / M
...
class QuaternionAttention_v2(nn.Module):
    ...
    def forward(self, queries, keys, values, query_omegas, query_thetas, key_omegas, key_thetas, attn_mask):
        ...
        Q_angles = query_omegas * self.query_pos + query_thetas # (B, L, H, E)
        K_angles = key_omegas * self.key_pos + key_thetas # (B, S, H, E)
        
        Q_cos, Q_sin = Q_angles.cos(), Q_angles.sin() # (B, L, H, E)
        K_cos, K_sin = K_angles.cos(), K_angles.sin() # (B, S, H, E)
        ...
        Q_rotation = torch.cat(
            [
                torch.einsum('blhe,blhm->blhme', Q_quaternion[0], Q_cos) - torch.einsum('blhe,blhe->blhe', Q_quaternion[1], Q_sin),
                torch.einsum('blhe,blhm->blhme', Q_quaternion[1], Q_cos) + torch.einsum('blhe,blhe->blhe', Q_quaternion[0], Q_sin),
                torch.einsum('blhe,blhm->blhme', Q_quaternion[2], Q_cos) + torch.einsum('blhe,blhe->blhe', Q_quaternion[3], Q_sin),
                torch.einsum('blhe,blhm->blhme', Q_quaternion[3], Q_cos) - torch.einsum('blhe,blhe->blhe', Q_quaternion[2], Q_sin),
            ], dim=-1
        ) # (B, L, H, E)

        K_rotation = torch.cat(
            [
                torch.einsum('bshe,bshm->bshme', K_quaternion[0], K_cos) - torch.einsum('bshe,bshe->bshe', K_quaternion[2], K_sin),
                torch.einsum('bshe,bshm->bshme', K_quaternion[1], K_cos) - torch.einsum('bshe,bshe->bshe', K_quaternion[3], K_sin),
                torch.einsum('bshe,bshm->bshme', K_quaternion[2], K_cos) + torch.einsum('bshe,bshe->bshe', K_quaternion[0], K_sin),
                torch.einsum('bshe,bshm->bshme', K_quaternion[3], K_cos) + torch.einsum('bshe,bshe->bshe', K_quaternion[1], K_sin),
            ], dim=-1
        ) # (B, S, H, E)
        
        scale = self.scale or 1. / sqrt(E)

        scores = torch.einsum("blhe,bshe->bhls", Q_rotation, K_rotation)
...
class InducedLearningtoRotateAttentionLayer(nn.Module):
    def __init__(self, query_size, key_size, d_model, n_heads, period_type='variant', n_periods=2, d_keys=None,
        ...
        super(InducedLearningtoRotateAttentionLayer, self).__init__()
        ...
        # attn_1 = FullAttention(self.inducing_size, key_size, mask_flag, scale, attention_dropout, output_attention)
        # attn_2 = FullAttention(query_size, self.inducing_size, mask_flag, scale, attention_dropout, output_attention)
